...
from routing_encoding import routing_encoding
...
def encode_time_related_features(event: Event, previous_event: Event, case_start_time: datetime, dwell_time_mapper: bidict, average_time_since_start_in_sec):
    """
    Encode features related to time
    """
    # normalized dwell time of current event
    dwell_time = event[constants.attr_arrived_time] - previous_event[constants.attr_arrived_time] if previous_event is not None else event[constants.attr_arrived_time] - case_start_time
    ...
    #time_since_case_start_of_eqtyp = time_since_case_start_mapper[event['EQTYP']]
...
def encode_event_attribute_features(event: Event, event_attribute_mappers: dict) -> [int]:
    ...
    event_attr_vector.append(last_four_digits) # Add the last four digits of EID as feature
...
def encode_state_features(case_state: [Event], case: Trace, average_time_since_start_in_sec: float, eid_mapper: bidict, dwell_time_mapper:bidict, event_attribute_mappers: dict, case_attribute_mappers:dict, network: nx.DiGraph) -> [[]]:
    """
    ...
    for idx, event in enumerate(case_state):
        ...
        previous_event = case_state[idx-1] if idx >= 1 else None 
        time_features = encode_time_related_features(event, previous_event, case.attributes['Init Datetime'], dwell_time_mapper, average_time_since_start_in_sec)
