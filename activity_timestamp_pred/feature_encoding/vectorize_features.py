...
from feature_encoding import feature_encoding
...
def vectorize_features(log: EventLog, average_time_since_start_in_sec: float, eid_mapper: bidict, dwell_time_mapper:bidict, event_attribute_mappers: dict, case_attribute_mappers:dict, max_case_len: int, network: nx.DiGraph) -> [[[]]]:
    """
    Vectorize the extracted features for each case in the event log
    """

    # Initialize an empty list to store the vectorized features
    X = []
    ...
    for case in log:
        ...
        for current_idx, event in enumerate(case[1:], 1):
            ...
            state_with_features = feature_encoding.encode_state_features(case[:current_idx], case, average_time_since_start_in_sec, eid_mapper, dwell_time_mapper, event_attribute_mappers, case_attribute_mappers, network)
            X.append(state_with_features)

        # Encode state features for the entire case (last state) and append them to X
        last_state_with_features = feature_encoding.encode_state_features(case, case, average_time_since_start_in_sec, eid_mapper, dwell_time_mapper, event_attribute_mappers, case_attribute_mappers, network)
        X.append(last_state_with_features)
...
def vectorize_rtm_prediction(log: EventLog, average_time_till_case_end: float) -> [float]:
    """
    Vectorize dwell time prediction for each case in the event log
    """
